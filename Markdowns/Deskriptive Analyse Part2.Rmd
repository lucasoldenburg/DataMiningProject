...
df$'YearsOfService_BASFBerlin' <- data_chars_as_num[3]
df$'YearsOfService_BASFall' <- data_chars_as_num[8]
...
head(df$`Months Since Last Development`)
...
```{r}
# df1 <- data.frame(df$Age, df$EmployeeChurned)
# # df1$age <- df$Age
# # df1$bla <- df$EmployeeChurned
# df$EmployeeChurned <- as.numeric(df$EmployeeChurned)
# pairs(df1, col = df1$EmployeeChurned)
```
