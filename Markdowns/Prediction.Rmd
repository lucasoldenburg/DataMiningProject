...
#cp -> Complexity Parameter definiert die minimal zu erreichende Verbesserung
#je Ast
...
>>>>>>> cc0dc6c10b6697416bd9074cf0328479778129bc
...
```{r}
confusionMatrix(factor(raw_test$predict), factor(raw_test$EmployeeChurned),
                positive = "1")
```
...
raw_test$predict_rpart_opti <-classify(raw_test$prob, threshold_rpart_opti)

confusionMatrix(factor(raw_test$predict_rpart_opti), factor(raw_test$EmployeeChurned),
                positive = "1")
```
...
h2oGrid_drf <- h2o.grid("drf", x = predictors, y = target,
            grid_id = "h2oGrid",
...
h2oGridPerf_drf <- h2o.getGrid(grid_id = "h2oGrid",
                           sort_by = "auc",
                           decreasing = TRUE)
...
confusionMatrix(factor(raw_testH2O_asdf$predict), factor(raw_testH2O_asdf$EmployeeChurned),
                positive = "1")
```
...
## Vergleich rpart und H2O D
```{r}
confusionMatrix(factor(raw_test$predict_rpart), factor(raw_test$EmployeeChurned),
                positive = "1")
