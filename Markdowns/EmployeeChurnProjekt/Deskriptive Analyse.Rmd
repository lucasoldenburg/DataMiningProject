...
Zur Vorhersage, ob eine Arbeitskraft wahrscheinlich das Unternehmen verlässt, kommen Algorithmen des überwachten maschinellen Lernens zum Einsatz. Es handelt sich um eine Klassifizierung zwischen zwei Klassen. Neben einem Entscheidungsbaum und Tree-Ensambles (Boosting, Bagging, möglicherweise Einsatz von XGBoost) wird die logistische Regression eingesetzt. Im dritten Modell soll die Vorhersage mittels einer Support Vector Maschine durchgeführt werden.

Die Ergebnisse der unterschiedlichen Modelle werden mittels der AUC (Area under the curve) und einer Confusion-Matrix miteinander verglichen. Zusätzlich zur Accuracy soll die Balanced Accuracy untersucht werden.

Um die einzelne Modelle der Realität anzupassen soll mit einer Kostenfunktion optimiert werden, welche Falsch-Positive-Werte (FP) und Falsch-Negative-Werte (FN) unterschiedlich bestraft.
...
Diese Kosten jedoch zu verallgemeinern, führt zu ungenauen und potenziell falschen Ergebnissen. Zudem ist für eine genaue Schätzung ein hoher Grad an Verständnis in dem Spezialbereich nötig. Dafür ist der Austausch mit den Experten der lokalen Personalabteilung des BASF Service Hubs Berlin geplant. Allgemein sollten jedoch die Falsch-Negativ-Klassifikationen minimiert werden.
...
### Set-Up 
Zu erst werden die benötigten Bibliotheken geladen. Die Daten werden anschließend aus einer .xlsx-Datei eingelesen.
```{r}
library(readxl)
library(writexl)
library(ggplot2)
...
```
Wie der Präsentation zur Projektdefiniton zu entnehmen ist, wurden die Daten mittels
des Tools Alteryx aus dem SAP Data Warehouse extrahiert. Dabei kamen fünf untschiedliche
Cubes als Datenbasis zum Einsatz. Darüber hinaus wurden mit Hilfe von zwei Mapping-
Tabellen die Positionen in Gehaltsbänder sowie die den Mitarbeitenden zugeordneten
Kostenstellen den Funktionsbereichen zugeordnet.
Basierend auf der Central Person ID (Unique Identifier) wurden die Tabellen miteinander
gejoined.
Aus den Zeitstemplen wurden Zeiträumbe berechnet und im Themengebiet Entwicklung kam
es zur Erstellung neuer Features aus den gegebenen Datenpunkten (Entwicklung von
tariflicher Beschäftigung in außertarifliche Beschäftigung, Anzahl an horizontalen
oder vertikalen Entwicklungen). 


Danach erfolgt die Umwandlung von Zahlenwerten in das Format Numeric und die Datumswerte in das Format Datum. 
```{r}
...
### EmployeeChurned
Dies ist das Ziel-Merkmal und beschreibt, ob die Person das Unternehmen verlassen hat (1) oder noch aktiv ist (0).
```{r}
...
```{r}
df$`Global contract type` <- replace(df$`Global contract type`,df$`Global contract type`== "apprentice","1")
...
colnames(df)[which(names(df) == "Global contract type")] <- "TemporaryPersonel"
qplot(df$`TemporaryPersonel`)
```
...
colnames(df)[which(names(df) == "Employee Subgoup text clustered")] <- "NonExempt"

unique(df$'NonExempt')
summary(df$'NonExempt')
sum(is.na(df$'NonExempt'))
qplot(df$'NonExempt')

df$`Employee Subgroup text` <- NULL
...
#Verlust der Datenpunkte vermeiden und Gehaltsband mit höchster
#Ausprägung angeben (Payband D)
df$Paybands2 <- df$Paybands %>% str_replace_na("D")

df$Paybands <- as.factor(df$Paybands2)
df$Paybands2 <- NULL
...
colnames(df)[colnames(df) == "start_employee subgroup detail clustered"] <- "start_NonExempt"

qplot(df$start_NonExempt)
...
Der Funktionsbereich wurde anhand der Kostenstelle aus einer weiteren Tabelle hinzugefügt. 
```{r}
sum(is.na(df$'decrease_Category'))
df$decrease_Category2 <- df$'decrease_Category' %>% str_replace_na("Other")

df$'decrease_Category' <- as.factor(df$'decrease_Category2')
df$'decrease_Category2'<- NULL

df$'decrease_Category' <- replace(df$'decrease_Category',df$'decrease_Category'== "IT Service CC","Other")
df$'decrease_Category' <- replace(df$'decrease_Category',df$'decrease_Category'== "GBR/N","Other")
df$'decrease_Category' <- replace(df$'decrease_Category',df$'decrease_Category'== "Intellectual Property","Other")
df$'decrease_Category' <- replace(df$'decrease_Category',df$'decrease_Category'== "RAA/A Competence","Other")
df$'decrease_Category' <- replace(df$'decrease_Category',df$'decrease_Category'== "Procurement Services - GP","Other")

unique(df$'decrease_Category')
summary(df$'decrease_Category')
qplot(df$'decrease_Category')
```
Aufgrund nicht mehr existierender Kostenstellen, Funktionsbereichen oder
nicht zugeordneten Kostenstellen zu Funktionsbereichen kommt es <NA>-Werten.
Diese werden zusammen mit wenig besetzten Funktionsbereichen geclustered.
Auch der Bereich "Procurement Services - GP" zählt dazu, da diese
Dienstleistung nicht mehr vom Hub erbracht wird.
...
Aus diesem Grund müssen diese (NaN) aus dem Datenset entfernt werden. Da es 895 
dieser Fälle gibt, wird dieses Merkmal als Ganzes nicht weiter betrachet.
```{r}
...
summary(df$Age)
sum(is.na(df$Age))
churned_employees <- subset(df, EmployeeChurned == 1)
current_employees <- subset(df, EmployeeChurned == 0)

mean(churned_employees$Age)
median(churned_employees$Age)

mean(current_employees$Age)
median(current_employees$Age)

qplot(df$Age)
ggplot(df, aes(EmployeeChurned, Age)) + 
  geom_boxplot(alpha=0.2)
```
Das Durchschnittsalter der aktiven Belegschaft ist geringer als das derer, die
...
df$`Month Of Service` <- pmax(df$`Month Of Service`, 0)
...
df_cor <- data.frame(df$Age, df$`Month Of Service`, df$`Months Since Last Development`, df$NonExempt, df$Paybands, df$start_NonExempt,df$EmployeeChurned)
df_cor$df.NonExempt <- as.factor(df_cor$df.NonExempt)
df_cor$df.start_NonExempt <- as.factor(df_cor$df.start_NonExempt)
pairs(df_cor[0:6], col = df_cor$df.EmployeeChurned)
```
...
Besonderer Fokus soll dabei auf der Unterteilung in Mitarbeiter:innen bleiben
oder verlassen das Unternehmen liegen sowie der Zugehörigkeitsdauer.

```{r}
ggplot(df, aes(`Month Of Service`, Age)) +
geom_jitter(aes(colour = EmployeeChurned), alpha=0.2, height = 0.3)
```
Der Standord Berlin hat allgemein ein geringes Durchschnittsalter.
Auch wenn die Anzahl der die im Unternehmen noch aktiven Mitarbeiter:innen die der gegangenen übersteigt, kann gesagt werden, dass ein Großteil nicht länger als
50 Monate aktiv war. 
Eine deutliche blaue Linie ist bei circa 24 Monaten zu erkennen. Viele Arbeit-
nehmer:innen orientieren sich in der Regel nach dieser Zeit um. Es kann sich
allerdings auch um befristete Verträge handeln.
Die erhöhte Dichte von blauen Punkten im Bereich ab 60 Jahren ist auf den
Renteneintritt zurückzuführen.

```{r}
ggplot(df, aes(TemporaryPersonel)) +
  geom_bar(aes(colour = EmployeeChurned), alpha=0.2)

ggplot(df, aes(TemporaryPersonel, `Month Of Service`)) + 
  geom_boxplot(aes(colour = EmployeeChurned), alpha=0.2)
```
Befristungen bis 12 Monate oder 24 Monate sind eindeutig zu erkennen.
Da seit 2021 stark auf die Hub-Entwicklung gesetzt wird, sind viele
Positionen geschaffen und ersteinmal mit befristeten Verträgen besetzt worden.


```{r}
#Gender 1 entspricht weiblich
ggplot(df, aes(Gender)) + 
  geom_bar(aes(colour = EmployeeChurned), alpha=0.2)

ggplot(df, aes(Gender,`Month Of Service`)) + 
  geom_boxplot(aes(colour = EmployeeChurned), alpha=0.2)
```
Im Unternehmen sind mehr als 60% der Belegschaft weiblich. Der Anteil derer, die
das Unternehmen verlassen ist bei dem männlichen Teil der Belegschaft höher, als
für den weiblichen. 
Während die Männer und Frauen, die das Unternehmen verlasssen dies nach ähnlicher 
Dauer machen, kann für die gebliebenen festgestellt werden, dass die weiblichen 
länger im Unternehmen bleiben.
Frauen bleiben demnach länger im Unternehmen und die Abwanderungsrate ist geringer.
...
str(df)
```
Der Anteil an Teilzeitmitarbeiter:innen entweder zum Start, als auch beim
letzten aktiven Zeitpunkt ist klar unterrepräsentiert. Es kann darüber hinaus
kein Muster aus den beiden vorhergehenden Visualisierungen erkannt  werden. 
...
ggplot(df, aes(Paybands, `Month Of Service`)) + 
  geom_boxplot(aes(colour = EmployeeChurned), alpha=0.2)
```
Mitarbeitende in höheren Paybands zum Zeitpunkt der letzten Aktivität sind länger 
im Unternehmen, als in unteren. Mitarbeitende, die das Unternehmen verlassen haben, waren in fast allen Paybands länger im Unternehmen als die aktiven. Dies kann auch daran liegen, dass solche Positionen auch nachbesetzt werden müssen, zum Teil von extern oder es rücken Mitarbeitende nach.

```{r}
ggplot(df, aes(decrease_Category)) +
  geom_bar(aes(colour = EmployeeChurned), alpha=0.2) +
...
Aus dem Bereich GB Management verlassen wenige Mitarbeiter:innen das
Unternehmen, was daran liegt, dass es sich eher um Senior-Positionen handelt und 
es eine neuere Einheit ist.
Die geringe Abwanderung im Bereich Safety kann darauf zurückgeführt werden,
dass es sich um eine relativ neue Einheit handelt.

"People" sowie "Supplier & Reporting" bilden die größten Bereiche.

```{r}
ggplot(df, aes(decrease_Category, `Month Of Service`)) + 
  geom_boxplot(aes(colour = EmployeeChurned), alpha=0.2)+
  theme(axis.text.x=element_blank())
```
X-Achse:
...
In den größeren Bereichen (Customer, People, Supplier & Reporting) ist zu erkennen, 
dass das Unternehmen enweder früh verlassen wird oder Mitarbeiter:innen deutlich
länger bleiben. (Median größer als 12 Monate auseinander für betreffende Bereiche)


```{r}
write_xlsx(df,"20220110_dataPrepped_beforeOneHot.xlsx")
```
...
write_xlsx(hot_df,"20220110_dataPrepped_afterOneHot.xlsx")
```
