...
rpart <- rpart(EmployeeChurned ~., y = TRUE, x = TRUE,
                  data=test_train)
test_test$prob <- rpart.predict(rpart, test_test)

#knnPrediction <- prediction(test_test$prob, test_test$EmployeeChurned)
knnPerf <- performance(knnPredict, "tpr", "fpr")
...
svmfit <- svm(EmployeeChurned ~ ., data = test_train[,cols], kernel = "radial", 
              cost = 0.01, gamma = 0.5, scale = TRUE)
...
#test_train$EmployeeChurned <- as.numeric(as.character(test_train$EmployeeChurned))
#test_train$ <- as.numeric(as.character(test_train$EmployeeChurned))
