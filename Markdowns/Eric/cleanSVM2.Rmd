...
### 1. Versuch svm durchzuführen mit caret und e1071

## Erstellen der Train und Test Datensets
```{r}
df_test <- subset(hot_df, select = c("EmployeeChurned", "Age", "Month Of Service"))
...
#predict
prediction_linear <- predict(linear_svm, testDF, type = "class")
    
# confusion matrix with quality measures
confusionMatrix(prediction_linear, testDF[, "EmployeeChurned"])
```
...
#predict
prediction_radial <- predict(radial_svm, testDF, type = "class")
    
# confusion matrix with quality measures
confusionMatrix(prediction_radial, testDF[, "EmployeeChurned"])
```

##SVM mit besten Parametern
Für H2O nutzen wir die ermittelten Gamma Werte von Linear, da dies der Typ ist, der von H2O genutzt wird. 
```{r}
#https://docs.h2o.ai/h2o/latest-stable/h2o-docs/data-science/svm.html
#SVM durchführen 
svm_model <- h2o.psvm(
...
best_perf_svm <- h2o.performance(best_svm_model_h20, testH2O)
best_perf_svm
```
...
#predict
pred <- h2o.predict(best_svm_model_h20, newdata = testH2O)
...
```
Die Fläche unterhalb der ROC-Kurve ist mit 0.59 größer als 0,5 und damit liefert 
